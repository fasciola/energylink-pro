...
export default function ElectricianMapPage() {
  ...
  // Adapt Firestore electrician profile into the minimal structure ElectricianMap expects
  const mapElectricians: MapElectrician[] = useMemo(() => {
    return electricians
      ...
      .map((e) => ({
        id: e.uid || (e as any).id,
        name: e.displayName || (e as any).name || "Electrician",
  ...
  const selected = useMemo(() => {
    ...
    return electricians.find((e) => (e.uid || (e as any).id) === selectedId) || null;
          ...
          {loading ? (
            ...
            <div className="text-gray-600">
              ...
              <div className="mt-2 text-xs text-gray-500">
                Tip: electricians must set <code>location.lat</code> and <code>location.lng</code> in Firestore.
              </div>
            </div>
          ) : (
              ...
              {mapElectricians.map((e) => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl border transition ${
                    selectedId === e.id ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-200"
                  }`}
                >
